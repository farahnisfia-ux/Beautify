...
$r = $conn->query("SELECT p.*,k.nama as kat FROM produk p JOIN kategori k ON p.kategori_id=k.id ORDER BY p.id DESC");
$produk = $r ? $r : null;
$r2 = $conn->query("SELECT * FROM kategori ORDER BY id");
...
<nav class="cat-nav">
  <div class="cat-nav-inner">
    <a href="/beautify/index.php" class="active">Home</a>
    <?php if($kategori_all): $kategori_all->data_seek(0); while($k=$kategori_all->fetch_assoc()): ?>
    <a href="/beautify/index.php?kat=<?=$k['id']?>"><?=$k['icon'].' '.htmlspecialchars($k['nama'])?></a>
    <?php endwhile; endif; ?>
