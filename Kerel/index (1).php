...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cart-quick-btn:hover {
            background: var(--pink);
            transform: scale(1.1);
        }
        .cart-quick-btn.added {
            background: var(--pink);
            opacity: 1;
        }
...
function saveCart() {
    sessionStorage.setItem('beautify_cart', JSON.stringify(cart));
}
...
function addToCart(id, name, brand, price, img) {
    ...
    if (existing) {
        ...
        cart.push({ id, name, brand, price: parseInt(price), img, qty: 1 });
    }
    saveCart();
    updateCartBadge();
    renderCart();
