...
// Summary stats
$_res = $conn->query("SELECT COUNT(*) as total_pesanan, SUM(total_harga) as revenue, AVG(total_harga) as avg_order FROM pesanan $base_where");
$stat = ($_res && $_res !== false) ? $_res->fetch_assoc() : null;
if (!$stat) $stat = ['total_pesanan' => 0, 'revenue' => 0, 'avg_order' => 0];

$_res2 = $conn->query("SELECT COUNT(*) as c FROM pesanan WHERE status='selesai' AND $where_date");
$selesai = ($_res2 && $_res2 !== false) ? (int)($_res2->fetch_assoc()['c'] ?? 0) : 0;

$_res3 = $conn->query("SELECT COUNT(*) as c FROM pesanan WHERE status='dibatalkan' AND $where_date");
$batal = ($_res3 && $_res3 !== false) ? (int)($_res3->fetch_assoc()['c'] ?? 0) : 0;
...
$produk_laris = $conn->query("
  ...
  WHERE p.status != 'dibatalkan' AND $where_date
...
for ($i = 29; $i >= 0; $i--) {
    ...
    $row = ($res && $res !== false) ? $res->fetch_assoc() : ['rev' => 0, 'cnt' => 0];
    $revenue_harian[] = ['hari' => date('d/m', strtotime("-$i days")), 'rev' => (float)($row['rev'] ?? 0), 'cnt' => (int)($row['cnt'] ?? 0)];
}
...
if (empty($kat_data)) {
    ...
    if ($res) { while ($r = $res->fetch_assoc()) $kat_data[] = $r; }
}
      ...
      <div class="dash-box">
        ...
        <div class="dash-box-body" style="padding:0">
          <table class="admin-table">
            ...
            <tbody>
              <?php if (!$top_user || $top_user->num_rows === 0): ?>
